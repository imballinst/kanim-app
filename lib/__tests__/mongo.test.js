...
import mongoFunctions from '../mongo';
...
let client;
let db;
// Assume getDatabaseConnection and closeDBConnection is always true
let counter = 2;
 ...
 * Generic Functions
 */
describe('mongo Generic Components (lib/mongo)', () => {
  /**
...
describe('mongo counter (lib/mongo)', () => {
  it('should test all functions', () => {
    expect(Object.keys(untestedFunctions).length).toBe(0);
    expect(counter).toBe(Object.keys(mongoFunctions).length);
  });
});
