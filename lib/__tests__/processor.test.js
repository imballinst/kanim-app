...
describe('Processor component (lib/processor)', () => {
    ...
    Offices: [
      {
        ...
        MO_ADDRESS:
          'Jalan Tengku M. Daud Beureuh No. 82, Beurawe, Kuta Alam, Kota Banda Aceh, Aceh 23373'
      },
      {
        MO_TELP: '(0645)-43039',
  ...
  it('should correctly filter offices', () => {
    ...
    return getFilteredOffices('kuki', 'Meulaboh').then(offices =>
  ...
  it('should correctly check availabilities and send email accordingly', () => {
    return deleteMany(db, 'notifications', {})
      ...
      .then(({ success, updatedIDs, expiredIDs }) => {
        return Promise.all([
          find(db, 'notifications', { _id: { $in: updatedIDs } }),
          find(db, 'notifications', { _id: { $in: expiredIDs } })
        ]);
      })
