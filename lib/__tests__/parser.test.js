import {
  isInMorning, countAvailable, buildDateRange, getQuotaDifference,
} from '../parser';
...
describe('Parser component (lib/parser)', () => {
  ...
  it('should correctly count total availability across morning and afternoon', () => {
    ...
    const array = countAvailable(availability);

    expect(array.length).toBe(1);
    expect(array[0].morning).toBe(60);
    expect(array[0].afternoon).toBe(30);
  });
  ...
  it('should correctly get the quota difference', () => {
    ...
    expect(getQuotaDifference(quota1, quota2, 'afternoon')).toBe('Siang: 0 -> 0');
  });
});
