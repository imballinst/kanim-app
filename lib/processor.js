...
const { NODE_ENV, KANIM_USERNAME: username, KANIM_PASSWORD: password } = require('../config/env');
const { parseJSONIfString } = require('./objectUtil');
const { winstonInfo } = require('./logging');
const { isBetween } = require('./dateUtil');
const { getAvailability, login, getOffices } = require('./request');
const { getOfficesQuota } = require('./quota');
const { find, updateMany } = require('./mongo');
const { sendMail } = require('./mail');

const getCookieAndToken = () =>
  login(undefined, username, password).then((res) => {
...
const getFilteredOffices = (cookie, nameFilter) =>
  getOffices(cookie).then(response =>
...
const getAvailabilities = (cookie, token, offices, dateRange) => {
  ...
  offices.forEach(({ MO_ID }) => {
    promises.push(getAvailability(cookie, token, MO_ID, startParams, endParams));
  });

  const officeNames = offices.map(({ MO_NAME }) => MO_NAME).join(', ');
  winstonInfo(`Checking availabilities for ${officeNames}...`);

  // [TODO]: Change Promise.all to another
        ...
        return Promise.all(mails)
          .then((responses) => {
            ...
            return Promise.all([
              ...
              // Update expired IDs
              updateMany(db, collection, { _id: { $in: expiredIDs } }, { $set: { expired: true } }),
            ]);
          })
